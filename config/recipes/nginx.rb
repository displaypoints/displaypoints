Capistrano::Configuration.instance(:must_exist).load do
  namespace :nginx do 
    desc "Generate the /etc/nginx/nginx.conf file"
    task :setup, :roles => :app do
      require 'erb'
      template = File.read(File.join(File.dirname(__FILE__), "templates", stage.to_s, "nginx.conf.erb"))
      result = ERB.new(template).result(binding)
      put result, "/tmp/nginx.conf", :mode => 0755
